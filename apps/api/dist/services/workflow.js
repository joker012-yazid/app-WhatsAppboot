...
exports.onJobStatusChange = onJobStatusChange;
exports.sendReminderMessage = sendReminderMessage;
exports.handleCustomerResponse = handleCustomerResponse;
 ...
 */
async function sendTemplateMessage(phone, templateKey, variables) {
    try {
        ...
        // Send message
        await (0, whatsapp_service_1.sendMessageToChat)(chatId, message);
 ...
 */
async function onJobStatusChange(jobId, newStatus, oldStatus) {
    try {
 ...
 */
async function sendReminderMessage(jobId, reminderType) {
    try {
        const job = await prisma_1.default.job.findUnique({
            where: { id: jobId },
            include: {
        ...
        if (sent) {
            ...
            await prisma_1.default.reminder.updateMany({
 ...
 */
async function handleCustomerResponse(phone, messageText) {
    try {
 ...
 */
async function handleAiResponse(phone, customer, messageText, job) {
    try {
        ...
        const chatId = phone.includes('@') ? phone : `${phone.replace(/\D/g, '')}@s.whatsapp.net`;
        await (0, whatsapp_service_1.sendMessageToChat)(chatId, aiResponse);
        // Log AI response
        await prisma_1.default.message.create({
            data: {
                customerId: customer.id,
 ...
 */
async function sendRegistrationConfirmation(jobId) {
    try {
        const job = await prisma_1.default.job.findUnique({
            where: { id: jobId },
            include: {
 ...
 */
async function handleNewCustomerAiResponse(phone, messageText) {
    try {
        const normalizedPhone = phone.replace(/\D/g, '');
        ...
        const previousMessages = isNewCustomer ? [] : (await prisma_1.default.message.findMany({
            where: { customerId: customer.id },
            orderBy: { createdAt: 'desc' },
