...
router.post('/', auth_1.requireAuth, (0, auth_1.requireRole)('ADMIN'), async (_req, res) => {
    const backup = await (0, backup_1.createBackup)({ manual: true });
    return res.status(201).json(backup);
});
router.get('/:filename/download', auth_1.requireAuth, (0, auth_1.requireRole)('ADMIN'), async (req, res) => {
    try {
...
router.post('/:filename/restore', auth_1.requireAuth, (0, auth_1.requireRole)('ADMIN'), async (req, res) => {
    try {
